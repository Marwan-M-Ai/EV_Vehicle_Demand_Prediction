...
st.markdown("""
    <style>
        ...
        h1, h2, h3 {
            color: #00D1FF !important; /* Electric blue headers */
        }

        h4, h5, h6 {
            color: #7FDBFF !important; /* Softer blue for subheadings */
        }
        ...
        /* Center content block background override (the one that's still white) */
        .block-container {
            background-color: transparent !important;
        }

        /* Optional: Container spacing cleanup */
        section.main > div {
            padding-top: 1rem;
        }

        /* Optional: Remove shadow from default boxes */
...
st.markdown("""
    <div style='text-align: center; font-size: 42px; font-weight: bold; color: #FFFFFF; margin-top: 40px;'>
        🔋 EV Adoption Forecast Tool for Washington State Counties
    </div>
""", unsafe_allow_html=True)
...
# Image
st.image("EV_Picture.jpg", use_container_width=True)
...
combined = pd.concat([historical_cum, forecast_df], ignore_index=True)
...
#display the "forecast complete sign
st.markdown("""
    <div style='
        text-align: center;
        margin-top: 30px;
        ...
        border-radius: 12px;
        padding: 18px 0;
        box-shadow: 0 2px 12px #00D1FF33;
    '>
        ✅ <span style="color:#00D1FF;">Forecast complete!</span> Explore the trends above.
    </div>
""", unsafe_allow_html=True)
...
if multi_counties:
    ...
    # Use visually distinct, bright colors for dark background
    color_map = {
        0: "#00D1FF",   # Electric blue
        1: "#FFDC00",   # Bright yellow
        2: "#FF4136",   # Vivid red
        3: "#2ECC40",   # Bright green (extra, if needed)
    }
    for idx, (cty, group) in enumerate(comp_df.groupby('County')):
        color = color_map.get(idx, "#FFFFFF")  # Default to white if more than 4 counties
        ax.plot(group['Date'], group['Cumulative EV'], marker='o', label=cty, color=color, linewidth=2)
...
if multi_counties and len(multi_counties) >= 2:
    st.markdown("""
        <div style='
            text-align: center;
            margin-top: 30px;
...
st.markdown("""
    <div style='
        ...
        font-weight: bold;
    '>
        Developed by <b>MARWAN M</b> for the AICTE Internship Cycle 2 (S4F Team)
    </div>
""", unsafe_allow_html=True)
